...
int TcpAccept(int fd, uint16_t port) {
  ...
  if ((s = accept(fd, (struct sockaddr *)&addr, &addrlen)) < 0) {
...
std::vector<work_unit> GenerateWork(Arrival a, double cur_us,
  ...
  while (true) {
    ...
    if (wtype == 1) {
      ...
      fprintf(stderr, "Tried to construct unsupported workload type\n");
    }
...
void PrePopulateMemcached(int threads, int max_key_idx) {
  ...
  for (int t = 0; t < threads; ++t) {
    th.emplace_back(std::thread([=] {
      int c = TcpDial(raddr, kMemcachedPort);
      if (c < 0) {
...
void ClientHandler(void *arg) {
  ...
  // Pre-populate the memcached server if GET operation is selected
  if (wtype == 2) {
    printf("Pre-populating memcached server with data...\n");
