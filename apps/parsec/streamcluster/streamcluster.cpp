...
_Thread_local struct drand48_data randBuffer;
#else
struct drand48_data randBuffer;
#endif

void init_rand_r(unsigned int seed) {
...
__attribute__((noinline)) float pkmedian(Points *points, long kmin, long kmax, long* kfinal,
                                         int pid, pthread_barrier_t* barrier);
...
class MemStream : public PStream {
public:
  MemStream(float* data_, long n_, int dim_) {
    data = data_;
    n = n_;
    dim = dim_;
    pos = 0;
  }
  size_t read( float* dest, int dim_, int num ) {
    size_t count = 0;
    if (dim_ != dim) {
      fprintf(stderr,"Dimension mismatch in MemStream read!\n");
      exit(1);
    }
    for( int i = 0; i < num && pos < n; i++ ) {
      memcpy(&dest[i*dim], &data[pos*dim], dim * sizeof(float));
      pos++;
      count++;
    }
    return count;
...
public:
  ...
  size_t read( float* dest, int dim, int num ) {
    ...
    for( int i = 0; i < num && n > 0; i++ ) {
      for( int k = 0; k < dim; k++ ) {
        long rand_val;
        lrand48_r(&randBuffer, &rand_val);
        dest[i*dim + k] = rand_val / (float)INT_MAX;
      }
      n--;
      count++;
    }
                   ...
                   long chunksize, long centersize, char* outfile )
{
  ...
  hrperf_pause();
  sleep(1); // make sure hrp stopped, we don't want to collect info for the final localSearch
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  ...
  srand48(SEED);
  init_rand_r(SEED); // Initialize random number generator state for the main thread

  PStream* stream;
  float* preGeneratedData = NULL;

  if (n > 0) {
    // Pre-generate all data before starting performance measurement
    preGeneratedData = (float*)malloc(n * dim * sizeof(float));
    if (preGeneratedData == NULL) {
      fprintf(stderr, "Not enough memory for pre-generated data!\n");
      exit(1);
    }
    ...
    for (long i = 0; i < n; i++) {
      for (int k = 0; k < dim; k++) {
        long rand_val;
        lrand48_r(&randBuffer, &rand_val);
        preGeneratedData[i * dim + k] = rand_val / (float)INT_MAX;
      }
    }
    // Use MemStream to read from pre-generated data
    stream = new MemStream(preGeneratedData, n, dim);
  } else {
  ...
  // Performance measurement starts inside streamCluster
...
void* localSearchSub(void* arg_) {
  init_rand_r(SEED + ((pkmedian_arg_t*)arg_)->pid); // Initialize random number generator state for this thread
...
float pspeedy(Points *points, float z, long *kcenter, int pid, pthread_barrier_t* barrier)
{
  ...
  if( pid != 0 ) { // we are not the master threads. we wait until a center is opened.
    ...
    for(i = 1; i < points->num; i++ )  {
      long rand_val;
      lrand48_r(&randBuffer, &rand_val);
      bool to_open = ((float)rand_val/(float)INT_MAX)<(points->p[i].cost/z);
      if( to_open )  {
        (*kcenter)++;
#ifdef ENABLE_THREADS
        pthread_mutex_lock(&mutex);
...
int selectfeasible_fast(Points *points, int **feasible, int kmin, int pid, pthread_barrier_t* barrier)
{
  ...
  *feasible = (int *)malloc(numfeasible*sizeof(int));
  ...
  for(int i=k1; i<k2; i++ ) {
    long rand_val;
    lrand48_r(&randBuffer, &rand_val);
    w = (rand_val/(float)INT_MAX)*totalweight;
    //binary search
    l=0;
    r=points->num-1;
    if( accumweight[0] > w )  {
